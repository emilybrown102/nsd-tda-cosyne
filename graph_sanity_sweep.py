#!/usr/bin/env python
r"""
graph_sanity_sweep_batch.py
===========================================================
Batch "PH-readiness" / graph sanity sweep for your REAL crossnobis RDMs.

✅ Auto-discovers matrices:
  D:\Emily\NSD_cogsci_sandbox\COGS401\COSYNE\RDMs_Final\subjects\subjXX\
    ROI-*_crossnobis_STRICT3_sessionSplit_raw.npy
...
✅ For EACH subject×ROI matrix (RAW only):
  ...
  3) epsilon sweep using DISTANCE quantiles on off-diagonal entries:
     - for each quantile q, eps = quantile(dist_offdiag, q)
     - same metrics as above

✅ Outputs:
  ...\RDMs_Final\graph_sanity_batch\
    - summary_knn.csv
    - summary_eps.csv
    - summary_matrix_checks.csv
    - WORST_CASE_* plots (a few subjects/ROIs with most fragmentation)
...
python graph_sanity_sweep_batch.py
===========================================================
"""
...
OUT_DIR = RDM_ROOT / "graph_sanity_batch"
...
# (these are usually what you want for VR/epsilon graphs)
...
def knn_undirected_edges(D: np.ndarray, k: int) -> list[tuple[int, int]]:
    """
    Undirected kNN:
      - for each i, connect to its k nearest j != i
      - take union of directed selections
    """
    n = D.shape[0]
    # guard: fill diagonal with +inf so it is never selected
    Dwork = D.copy()
...
def eps_threshold_edges(D: np.ndarray, eps: float) -> list[tuple[int, int]]:
    """
    Undirected epsilon graph: connect i<j if D[i,j] <= eps
    """
    n = D.shape[0]
...
ROI_RE = re.compile(r"ROI-([A-Za-z0-9]+)_crossnobis_STRICT3_sessionSplit_raw\.npy$")

def discover_rdms() -> list[tuple[int, str, Path]]:
    """
    Returns list of (subject_int, roi_name, filepath)
    """
    found = []
    ...
    for subj_dir in sorted(SUBJ_ROOT.glob("subj[0-9][0-9]")):
        ...
        for f in sorted(subj_dir.glob("ROI-*_crossnobis_STRICT3_sessionSplit_raw.npy")):
    ...
    if not found:
        raise RuntimeError(f"No matching RAW RDMs found under: {SUBJ_ROOT}")
    return found
...
def plot_knn_curve(rows: pd.DataFrame, out_png: Path, title: str) -> None:
    # rows has columns: k, n_components, largest_component
...
def plot_eps_curve(rows: pd.DataFrame, out_png: Path, title: str) -> None:
    # rows has columns: eps_quantile, n_components
    plt.figure(figsize=(7.2, 4.8))
...
def main() -> None:
    items = discover_rdms()
    print(f"Discovered {len(items)} RAW RDMs.")
    ...
    for subj, roi, fpath in items:
        ...
        sym_max_abs = float(np.max(np.abs(D - D.T))) if D.ndim == 2 else np.inf
        sym_ok = bool(sym_max_abs <= SYM_TOL)
        diag_max_abs = float(np.max(np.abs(np.diag(D)))) if D.ndim == 2 else np.inf
        diag_ok = bool(diag_max_abs <= DIAG_TOL)

        # Off-diagonal distribution (for eps quantiles)
    ...
    # ---------- Identify & plot worst cases ----------
    # "Worst" = most components at smallest k (k=2) OR most components at smallest eps quantile
    if not knn_df.empty:
        worst_knn = (
    ...
    for _, row in worst_knn.iterrows():
        ...
        out_png = fig_out / f"WORST_kNN_subj{subj:02d}_{roi}.png"
        plot_knn_curve(rows, out_png, title=f"subj{subj:02d} {roi} — kNN components vs k")
    for _, row in worst_eps.iterrows():
        subj, roi = int(row["subject"]), str(row["roi"])
        rows = eps_df[(eps_df["subject"] == subj) & (eps_df["roi"] == roi)].sort_values("eps_quantile")
        out_png = fig_out / f"WORST_eps_subj{subj:02d}_{roi}.png"
        plot_eps_curve(rows, out_png, title=f"subj{subj:02d} {roi} — eps components vs quantile")

    print(f"\nWorst-case plots saved to: {fig_out}")

    # ---------- Quick console summary ----------
    # Report any matrices that fail checks
    ...
    # Report typical connectivity at k=2 and eps_quantile=min
    if not knn_df.empty:
        tmp = knn_df[knn_df["k"] == min(K_LIST)]
        print("\n--- kNN (k=2) connectivity summary ---")
    ...
    if not eps_df.empty:
        ...
        print("\n--- Epsilon (lowest quantile) connectivity summary ---")
        print(tmp.groupby("roi")[["n_components", "largest_component", "avg_degree"]].median().round(3))
