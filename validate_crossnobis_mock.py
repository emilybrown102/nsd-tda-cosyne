#!/usr/bin/env python
r"""
validate_crossnobis_mock.py
===========================================================
PROF-PROOF END-TO-END VALIDATION FOR YOUR CROSSNOBIS PIPELINE
(file-based; same parquet schema; same computations)

What this validates
-------------------
This validates the *exact* steps of your real pipeline:
  1) STRICT filtering: 3 repeats in 3 distinct sessions + both parities
  2) Fold building: odd vs even sessions (averaging repeats within fold)
  3) Diagonal noise estimate from repeat residuals (no signal leakage)
  4) Diagonal whitening using w = 1/sqrt(var + eps)
  5) Crossnobis distance computation (2-fold; /P scaling)
  6) Across-ROI RSA similarity = correlation of ROI RDM upper triangles

Tests (designed to be unambiguous)
----------------------------------
TEST_0_deterministic_tinyNoise:
  - known structure + tiny noise
  - EXPECT: recovered RDM ~ weighted ground truth (corr ~ 1)

TEST_1_pure_noise:
  - no structure at all
  - EXPECT: recovered RDM mean ~ 0 (unbiased cross-validated distance)
    (fraction negative is reported; not used as strict pass/fail)
...
Across-ROI RSA (meaningfulness check)
-------------------------------------
We build ROIs with known relationships:
  ROI_A: base structure
  ROI_B: same as A  => corr(RDM_A, RDM_B) should be HIGH POSITIVE
  ROI_C: independent => corr(RDM_A, RDM_C) should be ~0
  ROI_D: permutation of A condition labels => corr(RDM_A, RDM_D) should be ~0

Permutation is a standard RSA sanity check:
it preserves the ROI's "within-ROI signal strength" but destroys alignment of conditions.
...
OUT_DIR = ROOT / "COGS401" / "COSYNE" / "RDMs_MockValidation"
...
N_CONDITIONS = 60     # increase if you want (e.g., 157 or 200)
P_FEATURES = 800

SESSIONS = [1, 2, 3]  # 3 repeats in 3 sessions; both parities exist
REPEAT_INDEX_BY_SESSION = {1: 0, 2: 1, 3: 2}
...
# =========================
# Pipeline functions (mirror your real code)
# =========================
...
def true_distance_rdm_weighted(mu: np.ndarray, w: np.ndarray) -> np.ndarray:
    """
    Ground truth matched to the pipeline:
    - we compare to the WHITENED (weighted) expected squared distance / P
    """
...
def write_mock_parquet(out_file: Path, subject: int, nsd_ids: List[str], spec: MockSpec, seed: int) -> Dict[str, np.ndarray]:
    rng = np.random.default_rng(seed)
    ...
    # ROI_B: identical geometry (strong + RSA similarity expected)
    mu_B = mu_A.copy()

    # ROI_C: independent geometry (~0 RSA similarity expected)
    mu_C = rng.normal(0, 1.0, size=(n, p)).astype(np.float32) if spec.structure == "two_cluster" else np.zeros((n, p), dtype=np.float32)

    # ROI_D: permute condition labels of A (destroys alignment; RSA ~ 0 expected)
...
def run_one(spec: MockSpec, seed: int, make_figures: bool) -> Dict:
    ...
    parquet_file = BETAS_OUT / f"subj{SUBJ_MOCK:02d}_{spec.name}_seed{seed}_mock.parquet"
    ...
    ids_strict = strict_images_3reps_3sessions_both_parities(df[df["ROI"] == "ROI_A"].copy(), nsd_ids)
    if len(ids_strict) != len(nsd_ids):
        raise RuntimeError(f"Strict filter kept {len(ids_strict)} / {len(nsd_ids)} (unexpected in this mock).")
    ...
    for roi in ROIS:
        ...
        X = build_session_split_folds(df_roi, ids_strict)
    ...
    if make_figures:
        plot_heatmap(
            ...
            title=f"{spec.name} (seed {seed}): ROI_A recovered crossnobis RDM [visual shift]",
            out_png=FIG_DIR / f"{spec.name}_seed{seed}_ROI_A_recovered.png",
            cmap=RDM_CMAP,
            cbar_label="Crossnobis (shifted for display)"
        )
        plot_heatmap(
            tA,
            title=f"{spec.name} (seed {seed}): ROI_A ground truth (weighted to match whitening)",
            out_png=FIG_DIR / f"{spec.name}_seed{seed}_ROI_A_truth_weighted.png",
            cmap=RDM_CMAP,
        ...
        plot_scatter(
            upper_tri_vec(tA),
            upper_tri_vec(rdmA),
            title=f"{spec.name} (seed {seed}): ROI_A recovered vs ground truth",
            out_png=FIG_DIR / f"{spec.name}_seed{seed}_ROI_A_scatter.png"
        )
        plot_heatmap(
            across_sim,
            title=f"{spec.name} (seed {seed}): Across-ROI RSA similarity (corr of RDMs)",
            out_png=FIG_DIR / f"{spec.name}_seed{seed}_acrossROI_corr_VALUES.png",
            cmap=SIM_CMAP,
            tick_labels=ROIS,
            show_values=True,
            value_fmt="{:.2f}",
...
def main():
    ...
    mc = []
    print(f"\nRunning Monte Carlo robustness: {len(seeds)} seeds ...")
    for s in seeds:
    ...
    # PASS/FAIL checks (now aligned with correct expectations)
    # NOTE: pure-noise fraction-negative is DESCRIPTIVE (not a hard fail).
    pf = {
        ...
        "TEST_1_pure_noise_mean_near_0": {
            "pass": abs(results_single["TEST_1_pure_noise"]["roi_A_rdm_summary"]["mean"]) < 0.05,
            "mean": results_single["TEST_1_pure_noise"]["roi_A_rdm_summary"]["mean"],
            "fraction_negative_descriptive": results_single["TEST_1_pure_noise"]["roi_A_rdm_summary"]["fraction_negative"],
        },
    ...
    payload = {
        ...
        "monte_carlo_spec": spec_mc.__dict__,
        "monte_carlo_summary": mc_summary,
        "pass_fail": pf,
        "interpretation_simple": [
            "When we inject known representational structure, recovered crossnobis RDM matches the expected (weighted) ground truth.",
            "When we inject pure noise (no structure), crossnobis distances average to ~0 (unbiased).",
            "Across-ROI RSA behaves meaningfully: same-geometry ROI_B matches ROI_A, independent ROI_C ~0, permuted-label ROI_D ~0."
        ],
    }
    ...
    lines.append("=== MONTE CARLO (moderate SNR; 20 seeds) ===")
    ...
    print("\nShow your prof these files:")
    print(" - metrics_summary.txt (PASS/FAIL + interpretation)")
    print(" - TEST_0_deterministic_tinyNoise_seed123_ROI_A_scatter.png (near y=x)")
    print(" - TEST_2_two_cluster_strongSignal_seed123_acrossROI_corr_VALUES.png (A~B high, A~C ~0, A~D ~0)")
