#!/usr/bin/env python
r"""
ph_batch_ripser_vr.py  (FIXED)
===========================================================
Persistent homology (H0/H1/H2) on CROSSNOBIS RDMs using ripser
(Vietoris–Rips filtration on a distance matrix).

Fixes vs previous version:
- ripser() cannot accept thresh=None in some builds.
  We ONLY pass thresh if we have a real float.

Careful handling:
- loads RAW crossnobis distance matrices:
    ROI-<roi>_crossnobis_STRICT3_sessionSplit_raw.npy
- checks: square, finite, symmetric, diagonal ~0
- shifts distances by a constant if negatives exist:
    D_shift = D - min_offdiag + eps
  (preserves ordering and persistence = death-birth)
- saves the exact matrix used for PH

Outputs:
  RDMs_Final/PH_VR_ripser/
    subjXX/
      ROI-<roi>/
...
OUT_ROOT = ROOT / "COGS401" / "COSYNE" / "RDMs_Final" / "PH_VR_ripser"
OUT_ROOT.mkdir(parents=True, exist_ok=True)

# You said you exclude 6 & 8 (and 6 had non-finite RDMs previously)
SUBJECTS = [1, 2, 3, 4, 5]
MAXDIM = 2
...
def discover_rdms_for_subject(subj: int) -> list[tuple[str, Path]]:
    ...
    if not out:
        raise RuntimeError(f"No RAW RDMs found for subj{subj:02d} in {subj_dir}")
    return out
